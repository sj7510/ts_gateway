...
class ClientProxyMock extends ClientProxy {
  ...
  send<TResult = any, TInput = any>(pattern: any, data: TInput): Observable<TResult> {
    ...
    if (pattern.cmd === 'get_user_by_id' && data['userId'] === '1') {
      return of({ id: 1, name: 'Test User', email: 'test@example.com' } as TResult);
    }
    ...
    return of({ success: true, message: 'Mock response', pattern, data } as TResult);
  }
...
describe('GatewayController (e2e)', () => {
  ...
  describe('GET /gateway/users/:id', () => {
    ...
    it('should handle error when microservice times out', () => {
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body).toHaveProperty('statusCode', HttpStatus.SERVICE_UNAVAILABLE);
  ...
  describe('POST /gateway/events', () => {
    it('should emit event and return success response', () => {
      const eventData = { type: 'user_created', payload: { id: 123, name: 'New User' } };
    ...
    it('should handle missing pattern with validation error', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(res.body.message.some(msg => msg.includes('pattern'))).toBe(true);
        });
    });
    ...
    it('should handle missing data with validation error', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body).toHaveProperty('message');
          expect(Array.isArray(res.body.message)).toBe(true);
          expect(res.body.message.some(msg => msg.includes('data'))).toBe(true);
        });
    });
    ...
    it('should handle non-object data with validation error', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body).toHaveProperty('message');
          expect(Array.isArray(res.body.message)).toBe(true);
          expect(res.body.message.some(msg => msg.includes('object'))).toBe(true);
        });
    });
